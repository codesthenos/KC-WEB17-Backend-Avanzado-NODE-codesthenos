...
export const validateQueryPrice = (priceString) => {
  ...
  const validPattern = /^(-?\d+(\.\d+)?(-\d+(\.\d+)?)?$|^\d+(\.\d+)?-$)/ // now accepts floating point numbers
...
export const validateQueryLimitSkip = (limitString, skipString) => {
  if (isNaN(limitString) || isNaN(skipString) || +limitString <= 0 || +skipString < 0) {
    throw createHttpError(400, '400 BAD REQUEST | ERROR in URL: LIMIT should be > 0 and SKIP >= 0')
  }
  return { limit: +limitString, skip: +skipString }
}

export const validateQueryFields = (fieldsString) => {
  const validPattern = /^([a-z]+(,[a-z]+)*)?$/
  if (!validPattern.test(fieldsString)) throw createHttpError(400, '400 BAD REQUEST | ERROR in URL: FIELDS query param should match <field1,field2,...> pattern')
  return fieldsString
}
