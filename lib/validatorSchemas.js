...
export const validatorProductMiddleware = ({ title, schema, errorHandler }) => (req, res, next) => {
  ...
  try {
    let tagsToValidate = ['lifestyle']
    let priceToValidate = '1'
    let nameToValidate = 'sikwhuwhepupiurnfupibuvbfubvuidfhpvhviuwfiuvwuinvwiufvnpwiufvpwiufbvpuiwb'
    if (body.tags) {
      tagsToValidate = typeof body.tags === 'string' ? [body.tags] : body.tags
    }
    if (body.price) {
      priceToValidate = body.price
    }
    if (body.name) {
      nameToValidate = body.name
    }
    schema.parse({ name: nameToValidate, price: priceToValidate, tags: tagsToValidate, image: req.file ? `/productsImages/${req.file.filename}` : imageToValidate })
  } catch (error) {
    if (req.url.startsWith('/api/')) {
      next(error)
    } else {
